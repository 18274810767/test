...
public class UserServiceImpl implements UserServiceI {
	...
	public DataGrid dataGrid(User user, PageHelper ph) {
		...
		if (l != null && l.size() > 0) {
			for (Tuser t : l) {
				...
				u.setCustomerNum(t.getTcustomers().size());
				
				Tsolarytype solarytype=t.getTsolarytype();
				if (solarytype != null) {
					u.setBz(t.getTsolarytype().getBz());
				}
				
				Torganization organization=t.getTorganization();
				if (organization != null) {
					u.setOrganizationName(t.getTorganization().getName());
				}
	...
	synchronized public void add(User user) throws Exception {
		...
		if (userDao.count("select count(*) from Tuser t where t.name = :name", params) > 0) {
			...
			u.setTsolarytype(solaryTypeDao.getById(user.getSolaryTypeId()));
			u.setTorganization(organizationDao.getById(user.getOrganizationId()));
			u.setCreatedatetime(new Date());
			u.setPwd(MD5Util.md5(user.getPwd()));
			userDao.save(u);
		}
	...
	public User get(String id) {
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("id", id);
		Tuser t = userDao.get("select distinct t from Tuser t left join fetch t.troles role left join fetch t.torganization organization where t.id = :id", params);
		User u = new User();
		BeanUtils.copyProperties(t, u);
		...
		if (t.getTorganization() != null && !t.getTorganization().getId().isEmpty()) {
			u.setOrganizationId(t.getTorganization().getId());
		}
	...
	public List<Tree> allTree(SessionInfo sessionInfo) {
		...
		l = userDao.find("select distinct t from Tuser t");
	...
	@Override
	public List exHSql(String str) {
		// TODO Auto-generated method stub
		return userDao.getCurrentSession().createQuery(str).list();
	}
}

...
<c:if test="${fn:contains(sessionInfo.resourceList, '/coursesController/editPage')}">
	<script type="text/javascript">
		$.canEdit = true;
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.resourceList, '/coursesController/delete')}">
	<script type="text/javascript">
		$.canDelete = true;
	</script>
</c:if>
<script type="text/javascript">
	...
	$(function() {
		dataGrid = $('#dataGrid').datagrid({
			url : '${pageContext.request.contextPath}/coursesController/dataGrid',
			fit : true,
			fitColumns : true,
			border : false,
			pagination : true,
		...
		parent.$.messager.confirm('询问', '您是否要删除当前用户？', function(b) {
			if (b) {
				...
				if (currentUserId != id) {
					...
					$.post('${pageContext.request.contextPath}/coursesController/delete', {
						id : id
					}, function(result) {
						if (result.success) {
	...
	function editFun(id) {
		...
		parent.$.modalDialog({
			...
			width : 500,
			height : 350,
			href : '${pageContext.request.contextPath}/coursesController/editPage?id=' + id,
			buttons : [ {
				text : '编辑',
				handler : function() {
	...
	function addFun() {
		parent.$.modalDialog({
			...
			width : 500,
			height : 350,
			href : '${pageContext.request.contextPath}/coursesController/addPage',
			buttons : [ {
				text : '添加',
				handler : function() {
					parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好

...
<script type="text/javascript">
	...
	$(function() {
		resourceTree = $('#resourceTree').tree({
			url : '${pageContext.request.contextPath}/userController/allTree',
			parentField : 'pid',
			//lines : true,
			checkbox : true,
			...
			onLoadSuccess : function(node, data) {
				var ids = $.stringToList('${organization.userIds}');
				if (ids.length > 0) {
				...
				$('#roleGrantLayout').layout('panel', 'west').panel('setTitle', $.formatString('[{0}]部门下面的用户', '${organization.name}'));
		...
		$('#form').form({
			url : '${pageContext.request.contextPath}/userController/grantOrganization',
			onSubmit : function() {
				...
				if (checknodes && checknodes.length > 0) {
					for ( var i = 0; i < checknodes.length; i++) {
						...
						if( checknodes.length > 1){
							$.messager.alert('Warning','只能选择一个负责人！');
							isValid=false;
							parent.$.messager.progress('close');
							return isValid;
						}
	...
	<div data-options="region:'west'" title="系统资源" style="width: 300px; padding: 1px;">
		<div class="well well-small">
			<form id="form" method="post">
				<input name="id" type="hidden" class="span2" value="${organization.id}" readonly="readonly">
...
<%-- 	<div data-options="region:'center'" title="" style="overflow: hidden; padding: 10px;">
		<div class="well well-small">
			<span class="label label-success">${organization.name}</span>
			<div>${organization.remark}</div>
		</div>

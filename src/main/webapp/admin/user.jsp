...
<c:if test="${fn:contains(sessionInfo.resourceList, '/userController/grantRole')}">
	<script type="text/javascript">
		$.canGrant = true;
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.resourceList, '/userController/grantOrganization')}">
	<script type="text/javascript">
		$.canGrant = true;
	</script>
</c:if>
...
<script type="text/javascript">
		...
		dataGrid = $('#dataGrid').datagrid({
			...
			columns : [ [{
				...
				formatter : function(value, row, index) {
					...
					str += '&nbsp;';
					if ($.canGrant) {
						str += $.formatString('<img onclick="grantFun(\'{0}\');" src="{1}" title="用户角色"/>', row.id, '${pageContext.request.contextPath}/style/images/extjs_icons/key.png');
					}
					str += '&nbsp;';
					if ($.canDelete) {
	...
	function grantFun(id) {
		...
		parent.$.modalDialog({
			...
			width : 500,
			height : 300,
			href : '${pageContext.request.contextPath}/userController/userRoleGrant?ids=' + id,
			buttons : [ {
				text : '授权',
				handler : function() {
					parent.$.modalDialog.openner_dataGrid = dataGrid;//因为授权成功之后，需要刷新这个dataGrid，所以先预定义好
	...
	function grantFun2(id) {
		...
		parent.$.modalDialog({
			...
			width : 500,
			height : 300,
			href : '${pageContext.request.contextPath}/userController/userOrganizationGrant?ids=' + id,
			buttons : [ {
				text : '授权',
				handler : function() {
					parent.$.modalDialog.openner_dataGrid = dataGrid;//因为授权成功之后，需要刷新这个dataGrid，所以先预定义好
